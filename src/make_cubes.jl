# SIDES Cube and Map Generation Pipeline (Julia Implementation)
# This script integrates WCS transformations, spatial/spectral binning, 
# and beam convolution to generate 2D maps and 3D datacubes.
...
function compute_histogram2d(y::AbstractVector, x::AbstractVector, y_edges::AbstractVector, x_edges::AbstractVector, weights::AbstractVector)
    ...
    for i in eachindex(x)
        # Bins are spaced exactly by 1.0 starting at -0.5:
...
"""
Helper function to compute a 3D weighted spatial-spectral histogram.
Replaces `numpy.histogramdd` with an O(N) native Julia version optimized 
for integer-aligned grid projections.
"""
...
# ==============================================================================
# SIDES EXTERNAL DATA/SED INTERPOLATION IMPLEMENTATION
# ==============================================================================
...
function gen_Snu_arr(lambda_list, SED_dict, redshift, LIR, Umean, Dlum, issb)
    ...
    Umean_min = SED_dict["Umean"][1] # Adjusted for 1-indexing correctly from Python
    dU = SED_dict["dU"]
    ...
    # Determine execution context (parallel or single-threaded)
    ...
    # Standard pmap fallback or normal map if single process
    ...
    # Expect Dlum to be pre-stripped or carry Mpc units
...
# ==============================================================================
# COORDINATE & WCS SYSTEM SETUPS
# ==============================================================================

"""
Generates uniform random RA and Dec coordinates in degrees for catalogs missing clustering.
"""
...
"""
Builds a 3D WCS Transform (RA, DEC, FREQ) and returns coordinate pixel mappings.
"""
...
# ==============================================================================
# CONVOLUTION KERNELS SETUP
# ==============================================================================

"""
Produces a list of N-channel 2D Gaussian convolution kernels corresponding 
to the telescope beam at each frequency channel.
"""
...
# ==============================================================================
# EXPORT UTILITIES (FITS SAVE IMPLEMENTATIONS)
# ==============================================================================

"""
Parses the WCS header string returned by WCSLIB into raw key/value/comment vectors.
Allows dynamic appending of duplicate COMMENT cards.
"""
...
"""
Saves 3D datacubes (nobeam, smoothed, and conversions) to FITS files with full WCS support.
"""
...
# ==============================================================================
# PIPELINE EXECUTION ENGINE (MAPS GENERATION)
# ==============================================================================
...
# ==============================================================================
# PIPELINE EXECUTION ENGINE (DATACUBES GENERATION)
# ==============================================================================

"""
Computes the monochromatic flux densities (Snu) across a specified frequency range 
for every galaxy in the catalog.
"""
function channel_flux_densities(cat::DataFrame, params_sides::Dict, params::Dict)
    ...
    println("Generate CONCERTO monochromatic fluxes...")
    Snu_arr = gen_Snu_arr(
        lambda_list, 
        SED_dict, 
        cat.redshift, 
...
"""
Builds a 3D continuum datacube, performing spatial 2D binning for every frequency channel.
"""
...
"""
Determines target WCS spectral channels for observed lines and computes 
the corresponding line flux densities in Jy.
"""
...
"""
Generates, smooths, and aggregates 3D datacubes for all CO transitions (J=1 to J=8).
"""
...
"""
Generates and smooths the 3D [CII] datacube based on a specified physical recipe.
"""
...
"""
Generates, smooths, and aggregates 3D datacubes for [CI10] and [CI21] transitions.
"""
